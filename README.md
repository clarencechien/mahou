...
| **名畫變色龍** | **名畫系／卡通系各 8 張**，8 關，畫裡藏會眨眼的小人——**第一關 1 隻，最後一關 24 隻**。手機當雷射筆（Wii 式指向，或觸控板）移游標蓋章搶快，DO 用 250ms 結算窗比 client 時間戳。像素大小與偽裝參數都在現場旋鈕上。名畫載不到自動退程序化抽象畫 |
...
- **Wikimedia 429**：短時間狂抓會被限流（開發時連抓 16 張中了兩次）。正常一關抓一張碰不到；真的中了走程序化 fallback

...
| **滑雪下坡** | 20–120 秒比距離。有**現場合成的電子配樂**（零音檔，升檔就加聲部＋加速）與跳躍／落地／撞擊／升檔音效。連點加速、傾斜轉向、抬起手機跳跳台。連段每 3 連升一檔，**三段檔位**：3 連「超加速」31 m/s、6 連「極速」38、9 連「神速」45——升檔那一下一檔踢得比一檔重（+6／+12／+18 surge），連段還在速度就一直停在高檔。沒抬手機輾過跳台只是小彈一下，而且 **0.7 秒內補抬還救得回來**。畫面依人數切成 1 / 2 / 2×2 / 4×2 / 4×3，最多 12 人同場 |
...
public/engine/
  ...
  bgm.js              #  程序化 chiptune 配樂（零音檔）：FC 配置、lookahead 排程、跟著檔位加聲部
...
tools/bgm-preview.mjs     # 把現場合成的配樂錄成 mp3 試聽，順便量每一檔的音符密度有沒有遞增

...
| **滑雪下坡** | 20–120 秒比距離。連點加速、傾斜轉向、抬起手機跳跳台。連段每 3 連升一檔，**三段檔位**：3 連「超加速」31 m/s、6 連「極速」38、9 連「神速」45——升檔那一下一檔踢得比一檔重（+6／+12／+18 surge），連段還在速度就一直停在高檔。沒抬手機輾過跳台只是小彈一下，而且 **0.7 秒內補抬還救得回來**。畫面依人數切成 1 / 2 / 2×2 / 4×2 / 4×3，最多 12 人同場 |
...
四款遊戲都有**現場合成的電子配樂與音效**（`public/engine/bgm.js`，零音檔、不下載任何東西）。
音樂跟著遊戲狀態走，這是錄好的檔做不到、也是不用 mp3／MIDI 的主要理由：滑雪升檔就加聲部＋加速、
**木頭人紅燈時整首音樂卡在原地**（音樂本身就是紅綠燈）、變色龍剩最後一兩隻才把時鐘滴答放進來、
砸罐子越後面的關卡越重。

音效分兩邊：**主控出配樂與全場共同的事件**（開始、結束、過關、喊聲），**自己的動作在自己手機上響**
（跳躍、落地、撞到、升檔、答對答錯）。理由是延遲——手機的動作走 手機→DO→主控 實測上行 p50 78ms，
主控發出來的是慢十分之一秒的回音。主控右上角 🔊 一鍵靜音，手機右上角也有自己的一顆。
...
4. 玩。要調手感就切到 **測試** 按 **⚙︎ 手感調校**，或直接按 **`T`**（見下）。點面板外面、按 `Esc` 或再按一次都會收起來。
   想看誰領先就按頂列的 **🏆 總分**——跨遊戲的名次點數（1st 10、2nd 7、3rd 5、4th 3、其他 1），
   算在 DO 上所以主控重整也不會歸零。**超過 6 人自動分兩欄**（實測 1080p 一欄只塞得下 6 列，
   而電視上沒有人會去捲名單）；每列左邊的色帶就是那個人在畫面上的角色顏色。
...
- 品牌、**🏆 總分**、遊戲選單、**開始**（不帶遊戲名，選單上就寫著了）
- 右邊四顆：**連線燈**（點了才展開右下的 DO 落點浮窗）、**⛶**、**正式／測試**一格開關、
  **🎵**（背景模式，開新分頁）
- **開打後**頂列只剩品牌＋停止（總分鈕跟著遊戲選單一起收起來）；
  左下的加入卡縮成一個房號（晚到的人點一下還是能放大 QR）
...
3. **每個玩家角色頭上都要有名字** —— 任何遊戲、任何視角，沒有例外。大螢幕上一堆同款小人，只靠配色認不出哪個是自己。名牌用 DOM 疊層而不是畫進 canvas（中文在低解析度像素緩衝裡會糊掉）。分割畫面另外要在每格左上角放該玩家的即時狀態。
   **配色只有 8 組但最多 12 人**，所以第 9 個人不能 `% 8` 繞回去跟第 1 個人同色——`CHARA.palFor(i)` 第二輪整組往亮推 42%（實測明度比 1.4–2.0:1），色相不變、認得出是誰。換在 `CHARA.sprite` 裡面，精靈／名牌／游標／總分板才會一致。
...
- **只設 ① 其實擋不到人。** `/api/room` 沒有 Access 就是對全世界開著的，
  任何人打一個 `POST /api/room` 就換得到憑證，然後想開幾間就開幾間。
  它擋掉的只有「連換憑證都不會的」那種亂打。**要真的鎖上就一定要配 ②。**
- ①＋②：只有你登入後換得到憑證，別人連 `/api/room` 都會吃 403。這才是鎖上的狀態。
...
public/engine/
  chara.js            #  程序化 32 單位角色（8 套玩家配色＋palFor 第二輪推亮、滑雪板、可旋轉）
  ...
  sfx.js              #  程序化 WebAudio 音效（零音檔）＋滑雪的跳躍／落地／撞擊／升檔。主控與手機共用
  bgm.js              #  程序化 chiptune 配樂（零音檔）：四首曲子、FC 配置、lookahead 排程、跟著遊戲狀態加聲部
...
tools/make-voice.mjs      # 用 Gemini TTS 把說明旁白錄成 public/voice/*.mp3（--sample 挑聲音、--only 補錄一段）

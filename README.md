...
| **背景模式**（不是遊戲） | `/ambient`。放音樂、跟歌詞、輪播照片。**不用 DO、不開房間**，開著一整場不產生費用。三種尺寸：全螢幕舞台／右下 widget（**預設**，跟隨模式固定兩行、只顯示一句）／縮小只剩歌名。卡片濃淡可調（`[ ]`，0–1，字不跟著淡；拉到 0 就只剩歌詞浮在照片上）。直式照片不裁，左右留一塊壓成灰的底；貼在白底上的照片會照設定檔裡的 `trim` 切掉白邊。歌單與背景都在一個 JSON 裡設定 |
